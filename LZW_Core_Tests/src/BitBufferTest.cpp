...
TEST_CASE("Write byte to buffer", "[BitBuffer]")
{
	...
	{
		BitBuffer buff{ 10 };
		buff.writeByte('\x40'); // 0100 0000 <-- insert order: 0 -> 1 -> 0 -> ...
...
TEST_CASE("Read all remaining bits")
{
	...
	CHECK_THAT(bits, Catch::Equals(std::vector<bool>{ 1, 1, 0, 1, 0 }));
}
...
TEST_CASE("Read all remaining bits as bytes")
{
	SECTION("Less than a byte")
	{
		...
		auto bytes = buff.readAllBytes(0); // 0111'0000
	...
	SECTION("Byte and some")
	{
		...
		CHECK_THAT(bytes, Catch::Equals(std::vector<byte_t>{ u'\xA7', u'\x80' }));
	}
}
